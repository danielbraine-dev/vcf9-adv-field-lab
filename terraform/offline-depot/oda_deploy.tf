...
variable "oda_vm_name"      { type = string }
variable "oda_mgmt_pg"      { type = string }
...
data "vsphere_network" "oda_net" {
  name          = var.oda_mgmt_pg
  datacenter_id = data.vsphere_datacenter.oda_dc.id
}

data "vsphere_resource_pool" "oda_rp" {
  name          = "${data.vsphere_compute_cluster.oda_cluster.name}/Resources"
  datacenter_id = data.vsphere_datacenter.oda_dc.id
}

# vSphere: Local Content Library for oda SE
resource "vsphere_content_library" "oda_se_cl" {
  name            = "oda SE Content Library"
  description     = "Local content library for oda SE artifacts"
